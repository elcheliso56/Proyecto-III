...
class empleados extends datos{
	...
	function consultar(){
		...
		$r = array();
		try{		
			$resultado = $co->query("Select * from empleados ORDER BY id DESC");
			if($resultado){	
				$respuesta = '';
				$n=1;
				...
				foreach($resultado as $r){
					...
					$respuesta = $respuesta."<td class='align-middle'>".$r['fecha_contratacion']."</td>";
					$respuesta = $respuesta."<td class='align-middle'>".$r['cargo']."</td>";
					$respuesta = $respuesta."<td class='align-middle'>".$r['salario']."</td>";
